...
//按照设计使用SDO1 = SDO2 = CSB3 = GND
//七位设备地址为ACC = 0x18, GYRO = 0x68, MAG = 0x10
...
enum ACCBW {    // define BMX055 accelerometer bandwidths//定义BMX055加速计带宽
...
// 2000hz ODR和未过滤(带宽523Hz)
  ...
  G_100Hz12Hz,
  G_200Hz64Hz,
  G_100Hz32Hz  // 100 Hz ODR and 32 Hz bandwidth// 100hz ODR和32hz带宽
};
...
//在beta参数中，准确性和响应速度之间有一个权衡。
//在最初的Madgwick研究中，0.041的beta值(对应于GyroMeasError 2.7°/s)给出了最佳精度。
//但是，有了这个值，LSM9SD0的响应时间大约是10秒到一个稳定的初始四元数。
//随后的变化也需要较长的滞后时间来稳定输出，这对四轴飞行器或机器人汽车来说不够快!
//通过增加beta (GyroMeasError) 15倍左右，响应时间常数减少到~2秒
//我没有注意到解的精度有任何下降。这本质上是PID控制中的第I个系数;
//反馈系数越大，解收敛得越快，通常是以牺牲精度为代价的。
//在任何情况下，这是Madgwick滤波融合方案中的自由参数。
//float beta = sqrt(3.0f / 4.0f) * GyroMeasError;   // compute beta
//float zeta = sqrt(3.0f / 4.0f) * GyroMeasDrift;   // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
#define Kp 2.0f * 5.0f // these are the free parameters in the Mahony filter and fusion scheme, Kp for proportional feedback, Ki for integral
//这些是Mahony滤波器和融合方案中的自由参数，Kp是比例反馈，Ki是积分
#define Ki 0.0f
 
// Declination at Sao Paulo, Brazil is -21 degrees 7 minutes on 2016-03-27
// 2016年3月27日，巴西圣保罗的气温为零下21度7分
